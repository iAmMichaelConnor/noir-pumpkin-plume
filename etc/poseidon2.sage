...
Field = GRUMPKIN_Fq # Think of this as Noir's native field. It's called Fq because we're thinking in terms of Grumpkin (instead of AltBN).
...
class Poseidon2Config:
    def __init__(self, t, rounds_f, rounds_p, internal_matrix_diagonal, round_constant):
        self.t = t
        self.rounds_f = rounds_f
        self.rounds_p = rounds_p
        self.internal_matrix_diagonal = internal_matrix_diagonal
        self.round_constant = round_constant
...
class BlackBoxResolutionError(Exception):
    pass
...
#    }
    ...
    def s_box(input):
        for i in range(len(input)):
            input[i] = Poseidon2.single_box(input[i])
...
#    }
    def add_round_constants(self, state, round_idx):
        for i in range(len(state)):
            state[i] += self.config.round_constant[round_idx][i]
...
#    }
    @staticmethod
    def matrix_multiplication_4x4(state):
        assert len(state) == 4
        t0 = state[0] + state[1]
        t1 = state[2] + state[3]
        t2 = state[1] * 2 + t1
...
#        }
#    }
    def internal_m_multiplication(self, state):
        total_sum = sum(state)
        for i in range(len(state)):
            state[i] = state[i] * self.config.internal_matrix_diagonal[i] + total_sum
...
#    ) -> Result<Vec<FieldElement>, BlackBoxResolutionError> {
    def permutation(self, inputs, length):
...
#        }
        if length != len(inputs):
            raise BlackBoxResolutionError(f"Length mismatch: expected {length}, got {len(inputs)}")
...
#            ));
#        }        
        if length != self.config.t:
            raise BlackBoxResolutionError(f"Expected {self.config.t} values, got {length}")
...
#            state[index] = *input;
#        }
        state = [Field(0) for _ in range(4)]
...
#        Self::matrix_multiplication_4x4(&mut state);
        # First linear layer
        self.matrix_multiplication_4x4(state)
...
#        }
        ...
        rf_first = self.config.rounds_f // 2
        for r in range(rf_first):
            self.add_round_constants(state, r)
            self.s_box(state)
            self.matrix_multiplication_4x4(state)
...
#        }
        # Internal rounds
        p_end = rf_first + self.config.rounds_p
        for r in range(rf_first, p_end):
            state[0] += self.config.round_constant[r][0]
            state[0] = self.single_box(state[0])
            self.internal_m_multiplication(state)
...
#        Ok(state.into())
        ...
        num_rounds = self.config.rounds_f + self.config.rounds_p
        for r in range(p_end, num_rounds):
            self.add_round_constants(state, r)
            self.s_box(state)
            self.matrix_multiplication_4x4(state)
...
#}
def poseidon2_permutation(inputs, length):
    poseidon = Poseidon2()
    return poseidon.permutation(inputs, length)
...
# Pass a list of field elements:
def poseidon2_hash(inputs):
    try:
        two_pow_64 = 18446744073709551616
        iv = len(inputs) * two_pow_64
        sponge = Poseidon2Sponge(iv, 3)
        ...
        raise BlackBoxResolutionError from e
...
#        result
#    }
    def __init__(self, iv, rate):
        self.rate = rate
        self.poseidon = Poseidon2()
        self.squeezed = False
        self.cache = []
        self.state = [Field(0) for _ in range(rate + 1)]
        self.state[rate] = iv
...
#        Ok(())
#    }
    def perform_duplex(self):
        try:
            # zero-pad the cache
            while len(self.cache) < self.rate:
                self.cache.append(Field(0))
            ...
            raise BlackBoxResolutionError(f"Error during duplex: {e}")
...
#    }
    def absorb(self, input):
        assert not self.squeezed, "Cannot absorb after squeezing"
        try:
            if len(self.cache) == self.rate:
                # If we're absorbing, and the cache is full, apply the sponge permutation to compress the cache
                self.perform_duplex()
                self.cache = [input]
            else:
            ...
            raise BlackBoxResolutionError(f"Error during absorb: {e}")
...
#    }
    def squeeze(self):
        assert not self.squeezed, "Sponge already squeezed"
        try:
            # If we're in absorb mode, apply sponge permutation to compress the cache
            self.perform_duplex()
            self.squeezed = True
            ...
            raise BlackBoxResolutionError(f"Error during squeeze: {e}")
...
class TestPoseidon(unittest.TestCase):
    ...
    def test_smoke(self):
        inputs = [Field(0)] * 4
        result = poseidon2_permutation(inputs, 4)
    ...
    def test_hash_smoke(self):
        ...
        result = poseidon2_hash(fields)
        expected_result = field_from_hex("130bf204a32cac1f0ace56c78b731aa3809f06df2731ebcf6b3464a15788b1b9")
        self.assertEqual(result, expected_result)
